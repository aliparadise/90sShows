...
namespace NinetiesTV
{
    class Program
    {
        ...
        **************************************************************************************************/
        ...
        {
            return shows
            .Where(s => s.StartYear >= 1990 & s.EndYear <= 1999) 
            .ToList();
        }
        ...
        {
            return shows.Where(s => s.Genres.Contains("Crime"))
            .OrderByDescending(s => s.ImdbRating)
            .TakeWhile(show => show.ImdbRating > 7)
            .Count();
        }
